...
def test_create_property_duplicate_name(property_repo):
    ...
    uc = CreatePropertyUseCase(property_repo)
    uc.execute("u1", "Unique", "Calle A", "Madrid", "28001", "ES", "apartment")
    
    with pytest.raises(ValueError, match="already exists"):
        uc.execute("u1", "Unique", "Calle B", "Madrid", "28001", "ES", "house")
...
def test_list_properties(property_repo):
    ...
    create_uc.execute("u1", "Piso A", "Calle A", "Madrid", "28001", "ES", "apartment")
    create_uc.execute("u1", "Casa B", "Calle B", "Sevilla", "41001", "ES", "house")

    list_uc = ListPropertiesUseCase(property_repo)
    props = list_uc.execute("u1")
    assert len(props) == 2
...
def test_record_income(property_repo, income_repo):
    ...
    create_uc = CreatePropertyUseCase(property_repo)
    prop = create_uc.execute("u1", "Piso", "Calle X", "Madrid", "28001", "ES", "apartment")
...
def test_get_profit_report(property_repo, income_repo, expense_repo):
    ...
    # Registrar ingresos
    income_uc = RecordIncomeUseCase(property_repo, income_repo)
    income_uc.execute("u1", prop.id, Decimal("750.00"), date(2026, 7, 1), "rent")
    income_uc.execute("u1", prop.id, Decimal("750.00"), date(2026, 8, 1), "rent")

    # Registrar gastos
    expense_uc = RecordExpenseUseCase(property_repo, expense_repo)
    expense_uc.execute("u1", prop.id, Decimal("200.00"), date(2026, 7, 5), "repair")

    # Calcular profit
    profit_uc = GetPropertyProfitReportUseCase(property_repo, income_repo, expense_repo)
    result = profit_uc.execute("u1", prop.id)
    # 750 + 750 - 200 = 1300
